...
class _ProfileScreenState extends State<ProfileScreen> {
  final int _currentIndex = 3; // Index 3 correspond à l'onglet profil
  ...
  void initState() {
    super.initState();
    // Charger les statistiques du profil au démarrage
 ...
 // Extrait de ProfileScreen pour la fonction _submitPasswordChange
...
void _submitPasswordChange(BuildContext context) {
  ...
  // Vérifier que l'utilisateur est connecté avant de procéder
  ...
  // Envoyer l'événement de changement de mot de passe
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // Bouton pour afficher/cacher le formulaire de changement de mot de passe
                    ElevatedButton(
                      onPressed: () {
                        setState(() {
  ...
  listener: (context, state) {
    if (state.status == AuthStatus.error) {
      // Erreur générale (probablement utilisateur non connecté)
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(state.errorMessage ?? 'Une erreur est survenue'),
          backgroundColor: Colors.red,
      ...
      // L'utilisateur est toujours authentifié mais il y a eu une erreur
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
